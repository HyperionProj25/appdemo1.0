...
import { getPitcherForPlayer } from '../data/csvStats'

export default function PlayerPitchingMetrics() {
  const { playerId } = useParams()
  const navigate = useNavigate()
  const player = getPlayer(playerId || '3000002')
  const [activeTab, setActiveTab] = useState('fb-velocity')
  const [showAnnotate, setShowAnnotate] = useState(false)
  const [annotations, setAnnotations] = useState([
    { date: '9/25', label: 'Bullpen session' },
    { date: '10/18', label: 'Mound work' },
  ])
  const [notes, setNotes] = useState([
    { date: '9/13/25', text: 'STARTED TRAINING AT KPI' },
    { date: '9/25/25', text: 'NEW PITCHING MECHANICS WITH COACH REYES' },
  ])
  ...
  const tabConfig: Record<string, {
    ...
    'fb-velocity': {
      points: pitcher.fbVeloTrend,
      yMin: 78, yMax: 100,
      yTicks: [80, 85, 90, 95, 100],
      unit: ' MPH',
    },
    'usage': {
      points: pitcher.usageTrend,
      yMin: 40, yMax: 80,
      yTicks: [45, 55, 65, 75],
      unit: '%',
    },
    'weak-contact': {
      points: pitcher.weakContactTrend,
      yMin: 15, yMax: 50,
      yTicks: [20, 30, 40, 50],
      unit: '%',
    },
  ...
  const cfg = tabConfig[activeTab] || tabConfig['fb-velocity']
