  ...
  return (
    <div style={{ display: 'flex', height: '100%', width: '100%', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '20px 28px 14px' }}>
        {/* Header */}
        <div className="anim-fade-in" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 14, flexShrink: 0 }}>
        ...
        <div className="anim-fade-in anim-delay-1" style={{ marginBottom: 16, flexShrink: 0 }}><button className="btn" disabled title="Coming soon">DATE</button></div>

        {/* Summary: Jump Visual + Stats */}
        <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexShrink: 0 }}>
          {/* MAX JUMP HEIGHT card — wider */}
          <div className="anim-slide-up anim-delay-2 card-glow" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '18px 22px', width: 320, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Stats card */}
          <div className="anim-slide-up anim-delay-3" style={{ flex: 1, background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '18px 22px' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 700, textTransform: 'uppercase' as const, lineHeight: 1.4, marginBottom: 10 }}>
        ...
        {/* Chart tabs */}
        <div className="anim-fade-in anim-delay-4" style={{ display: 'flex', border: '1px solid var(--orange-border)', borderRadius: 8, overflow: 'hidden', marginBottom: 16, flexShrink: 0 }}>
          {[
        ...
        <div className="anim-slide-up anim-delay-5" style={{ flex: 1, minHeight: 0, background: 'var(--card-bg)', border: '1px solid var(--orange-border)', borderRadius: 8, padding: '18px', boxShadow: 'inset 0 0 20px rgba(224,172,68,0.04)', display: 'flex' }}>
          <AnimatedChart
      ...
      {showAI ? (
        ...
        <button style={{ position: 'fixed', bottom: 20, right: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, background: 'var(--panel)', border: '1px solid var(--orange-border)', borderRadius: 10, padding: '14px 18px', cursor: 'pointer', zIndex: 100 }} onClick={() => setShowAI(true)}>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' as const, textAlign: 'center' as const, lineHeight: 1.3, color: 'var(--text)' }}>TALK TO<br />YOUR DATA</span>
...
const secHead: React.CSSProperties = { fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: '1.2px', color: 'var(--text)', marginBottom: 12, paddingBottom: 8, borderBottom: '1px solid var(--orange-border)', flexShrink: 0 }
const statLabel: React.CSSProperties = { fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 12, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--muted)', marginBottom: 5 }
const statVal: React.CSSProperties = { fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 22, color: 'var(--text-bright)', background: '#1e1e1e', borderRadius: 8, padding: '7px 10px', border: '1px solid #2a2a2a' }
